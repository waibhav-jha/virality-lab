    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between border-b border-white/10 pb-3 font-mono-tech text-[10px] text-[#7E8798] uppercase tracking-widest">
        <div className="flex items-center gap-2">
          <span className="text-[#D4FF00] font-bold">06 // VARIANT BENCHMARK</span>
          <span>::</span>
          <span>CANDIDATE VARIANT COMPARISON WORKBENCH</span>
        </div>
        <span>{allVariants.length || (winner ? 1 : 0)} SYNTHESIZED VARIANTS</span>
      </div>

      {/* Grid of Variants */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5" role="list" aria-label="Content variants">
        {/* Left Column (or 7 cols): Strongest Variant Candidate */}
        {winner && (
          <div
            className="lg:col-span-7 bg-[#07080A] border border-[#D4FF00]/60 p-5 sm:p-6 flex flex-col justify-between gap-4 font-mono-tech relative"
            role="listitem"
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <div className="flex items-center gap-2">
                <span className="bg-[#D4FF00] text-[#07080A] px-2 py-0.5 text-[10px] font-bold uppercase">
                  OPTIMAL SPECIMEN VARIANT
                </span>
                <span className="text-[10px] text-white/50">{winner.strategy || 'STRATEGY: REFINED HOOK'}</span>
              </div>
              <div className="flex items-baseline gap-1">
                <span className="font-display font-black text-2xl text-[#D4FF00]">{bestScore}</span>
                <span className="text-xs text-[#D4FF00]">/100 (+{delta > 0 ? delta : 0})</span>
              </div>
            </div>

            {/* Specimen Content Text */}
            <div className="bg-white/[0.02] border border-white/10 p-4 font-sans text-xs sm:text-sm text-[#F4F6F8] leading-relaxed">
              {winner.caption || winner.hook || 'No preview caption available.'}
            </div>

            {/* Changes Summary */}
            {winner.changes_summary && (
              <div className="p-3 bg-white/[0.02] border-l border-[#D4FF00] text-xs text-[#9DA7B8] font-sans">
                <span className="font-mono-tech text-[10px] text-[#D4FF00] uppercase block font-bold mb-0.5">
        ...
        <div className="lg:col-span-5 flex flex-col gap-3 font-mono-tech">
          <div className="text-[10px] text-[#7E8798] uppercase border-b border-white/10 pb-1">
            ALTERNATIVE HYPOTHESIS VARIANTS
          </div>

          {otherVariants.length === 0 ? (
            <div className="p-4 bg-white/[0.01] border border-white/5 text-xs text-[#5B6474]">
            ...
            otherVariants.map((variant, idx) => {
              ...
              return (
                <div
                  ...
                  className="bg-white/[0.01] hover:bg-white/[0.03] border border-white/10 p-4 flex flex-col gap-2 transition-all"
                  role="listitem"
                  aria-label={`Variant: ${variant.strategy || `#${idx + 2}`}`}
                >
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold text-white uppercase">{variant.strategy || `VARIANT 0${idx + 2}`}</span>
                    <div className="flex items-baseline gap-1">
                      <span className="font-display font-bold text-white">{varScore}</span>
                      {varDelta > 0 && (
                        <span className="text-[10px] text-[#D4FF00]">+{varDelta}</span>
                      )}
                    </div>
                  </div>

                  <p className={`text-xs text-[#9DA7B8] font-sans leading-relaxed ${isExpanded ? '' : 'line-clamp-2'}`}>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-white/5 text-[10px]">
                    <button
                      ...
                      className="text-[#5B6474] hover:text-white cursor-pointer uppercase"
                    >
                    ...
                    <button
                      ...
                      className="text-[#D4FF00] hover:underline cursor-pointer uppercase font-bold"
                    >
                      APPLY VARIANT →
                    </button>
                  </div>
