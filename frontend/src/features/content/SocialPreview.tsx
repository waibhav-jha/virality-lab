...
export const SocialPreview: React.FC<SocialPreviewProps> = ({
  ...
  return (
    <div className="w-full flex flex-col gap-2 text-left" aria-label="Specimen Examination Monitor">
      ...
      <div className="flex items-center justify-between border-b border-white/10 pb-2 font-mono-tech text-[10px] text-[#7E8798] uppercase">
        <div className="flex items-center gap-2">
          <span className="text-[#D4FF00] font-bold">SPECIMEN MONITOR</span>
          <span>::</span>
          <span className="text-white/80">{meta.label}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="px-1.5 py-0.2 bg-white/[0.04] border border-white/10 text-white">
            {meta.ratio}
          </span>
        </div>
      </div>
      ...
      <div className="relative w-full aspect-[4/5] sm:aspect-[9/14] max-h-[500px] bg-[#07080A] border border-white/15 corner-ticks overflow-hidden flex flex-col justify-between p-4">
        ...
        <div className="relative z-20 flex items-center justify-between font-mono-tech text-[10px] text-white/50">
          <div className="flex items-center gap-2">
            <span className="w-1.5 h-1.5 bg-[#D4FF00] animate-pulse" />
            <span className="text-white font-semibold">FEED_LIVE_STREAM</span>
          </div>
          <div className="flex items-center gap-2 text-[#7E8798]">
            <span>TC: 00:00:03:12</span>
            <span>|</span>
            <span className="text-[#D4FF00]">HOOK EVAL</span>
          </div>
        </div>

        {/* Media Background Content or Typography Grid */}
        <div className="absolute inset-0 z-0 flex items-center justify-center overflow-hidden bg-[#07080A]">
          {mediaUrl ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center relative">
              ...
              <span className="font-mono-tech text-xs text-[#7E8798] uppercase tracking-wider max-w-[240px]">
                NO MEDIA ATTACHED // EVALUATING HOOK & TRANSCRIPT PROFILE
              </span>
            </div>
          )}
          {/* Subtle Dark Vignette for Text Contrast */}
          <div className="absolute inset-0 bg-gradient-to-t from-[#07080A] via-black/40 to-[#07080A]/60 pointer-events-none" />
        </div>

        {/* Specimen Inspection Overlay */}
        <div className="relative z-20 flex flex-col gap-2 mt-auto">
          {/* Diagnostic Parameter Pill */}
          <div className="flex items-center gap-2 font-mono-tech text-[9px] text-[#7E8798] uppercase">
            <span className="bg-white/10 px-1.5 py-0.5 text-white">TARGET: {meta.targetSec}</span>
            <span>·</span>
          ...
          <div className="bg-[#07080A]/85 backdrop-blur-xs border border-white/10 p-3 max-h-36 overflow-y-auto">
            <p className="text-xs sm:text-sm text-[#F4F6F8] font-sans leading-relaxed">
              {caption.trim() ? (
                caption
              ) : (
                <span className="text-[#5B6474] font-mono-tech text-xs italic">
