    ...
    <section className="w-full cyber-card corner-ticks p-6 sm:p-8 text-left flex flex-col gap-6" aria-label="Audience response spectrum">
      ...
      <div className="flex flex-wrap items-center justify-between border-b-2 border-[#00FF41]/20 pb-3 font-mechanismo text-[11px] text-[#8E9E90] uppercase tracking-widest">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white/90 font-bold">MULTI-AGENT RESPONSE SPECTRUM</span>
        </div>
      ...
      <div className="flex flex-col gap-2.5 font-mechanismo">
        <div className="flex items-center justify-between text-xs text-[#8E9E90] pb-1 border-b border-[#00FF41]/20 uppercase font-bold">
        ...
        {sorted.map((persona, rk) => {
          ...
          return (
            <div key={persona.persona_id || rk} className="flex flex-col border-2 border-[#00FF41]/20 bg-[#000000]/90 shadow-[2px_2px_0px_0px_#000]">
              <button
                type="button"
                onClick={() => setExpandedId(isExpanded ? null : persona.persona_id)}
                className="p-3 sm:p-4 flex items-center justify-between gap-4 cursor-pointer hover:bg-[#080D09] transition-colors text-left"
                aria-expanded={isExpanded}
              >
                {/* Left Persona ID & Name */}
                <div className="flex items-center gap-3 min-w-0 w-48 sm:w-64">
                  <span className="text-xs text-[#00FF41] font-black">[{String(rk + 1).padStart(2, '0')}]</span>
                  <div className="flex flex-col truncate">
                    <span className="font-black text-white text-xs sm:text-sm uppercase tracking-wide font-csmigrate truncate">
                      {persona.persona_name}
                    </span>
                    <span className="text-[10px] text-[#8E9E90] font-bold">
                ...
                <div className="flex-1 max-w-xs sm:max-w-md hidden sm:flex flex-col gap-1">
                  <div className="flex justify-between text-[10px] text-[#8E9E90] font-bold">
                  ...
                  <div className="h-2 w-full bg-[#000000] border border-[#00FF41]/30 overflow-hidden">
                    <div
                      className={clsx(
                        'h-full transition-all duration-500',
                        isHigh ? 'bg-[#00FF41] shadow-[0_0_8px_#00FF41]' : isLow ? 'bg-[#FF0055]' : 'bg-[#00F0FF]'
                      )}
                  ...
                  <span
                    className={clsx(
                      ...
                      persona.stop_scroll_bool
                        ? 'text-[#000000] border-[#00FF41] bg-[#00FF41]'
                        : 'text-white border-[#FF0055] bg-[#FF0055]'
                    )}
                  >
                    {persona.stop_scroll_bool ? '[ SCROLL HALTED ]' : '[ SWIPE-AWAY ]'}
                  </span>
                  {isExpanded ? <ChevronUp className="w-4 h-4 text-[#00FF41]" /> : <ChevronDown className="w-4 h-4 text-white/40" />}
                </div>
              </button>

              {/* Expanded Detail Panel */}
              {isExpanded && (
                <div className="p-4 sm:p-5 border-t border-[#00FF41]/20 bg-[#050805] flex flex-col gap-4">
                  {/* Persona Reasoning Quote */}
                  {persona.reasoning && (
                    <div className="p-3 bg-[#000000]/80 border-l-2 border-[#00FF41] text-xs sm:text-sm italic text-[#FFFFFF] font-sans">
                      "{persona.reasoning}"
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 text-xs">
                    <div className="p-2 border border-[#00FF41]/20 bg-[#000000]/60 flex flex-col gap-1">
                      <div className="flex items-center gap-1 text-[10px] text-[#8E9E90]">
                    ...
                    <div className="p-2 border border-[#00FF41]/20 bg-[#000000]/60 flex flex-col gap-1">
                      <div className="flex items-center gap-1 text-[10px] text-[#8E9E90]">
                        <MessageSquare className="w-3 h-3 text-[#00FF41]" />
                        <span>COMMENT</span>
                      </div>
                    ...
                    <div className="p-2 border border-[#00FF41]/20 bg-[#000000]/60 flex flex-col gap-1">
                      <div className="flex items-center gap-1 text-[10px] text-[#8E9E90]">
                        <Share2 className="w-3 h-3 text-[#00F0FF]" />
                        <span>SHARE/DM</span>
                      </div>
                    ...
                    <div className="p-2 border border-[#00FF41]/20 bg-[#000000]/60 flex flex-col gap-1">
                      <div className="flex items-center gap-1 text-[10px] text-[#8E9E90]">
                        <Bookmark className="w-3 h-3 text-[#00FF41]" />
                        <span>SAVE POST</span>
                      </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                    {persona.strengths && persona.strengths.length > 0 && (
                      <div className="flex flex-col gap-1">
                        <span className="text-[10px] text-[#00FF41] font-bold uppercase">[+] Resonant Signals:</span>
                        <ul className="text-[#8E9E90] font-sans space-y-0.5">
                    ...
                    {persona.weaknesses && persona.weaknesses.length > 0 && (
                      <div className="flex flex-col gap-1">
                        <span className="text-[10px] text-[#FF0055] font-bold uppercase">[-] Friction Signals:</span>
                        <ul className="text-[#8E9E90] font-sans space-y-0.5">
